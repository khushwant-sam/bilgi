...
jQuery(document).ready(function() {
    ...
    // TAR handler
    jQuery('#uploadFormTAR').submit(function(e) {
      e.preventDefault();
    ...
    // ZIP handler
    jQuery('#uploadFormZIP').submit(function(e) {
      e.preventDefault();
    ...
    // Unity handler
    jQuery('#uploadFormUnity').submit(function(e) {
      e.preventDefault();
      var options = {
      type: 'POST',
      url: ajaxurl,
    ...
    jQuery("#tarfile").change(function ()  {
      myarcade_check_file_size("#tarfile", "#lbltarfile");
    });
    jQuery("#zipfile").change(function ()  {
      myarcade_check_file_size("#zipfile", "#lblzipfile");
    });
    jQuery("#unityfile").change(function ()  {
...
function showResponseTAR(data, statusText, xhr, $form)  {
  ...
  if ( ! data ) {
    alert("<?php _e("Error: Can not import that game!", 'myarcadeplugin'); ?>");
    return false;
  }
  ...
  if (statusText == 'success' && data.error == '') {
    ...
    if ( data.leaderboard_enabled ===  "1" ) {
      jQuery('#lbenabled').prop('checked', true);
    }
    
    jQuery("select#highscoretype").val(data.highscore_type);
...
function showResponseZIP(data, statusText, xhr, $form)  {
  ...
  if (statusText === 'success' && data.error === '') {
    ...
    if ( data.type === 'mochi' ) {
      jQuery('#gamedescr').val( data.description );
      jQuery('#gameinstr').val( data.instructions );
      jQuery('#gametags').val( data.tags );
      jQuery('.gamecat' + data.categs).prop('checked', true);
      jQuery('#importgametag').val( data.game_tag);
      if ( 'screen1_url' in data) { jQuery('#importscreen1').val( data.screen1_url ); }
      if ( 'screen2_url' in data) { jQuery('#importscreen2').val( data.screen2_url ); }
      if ( 'screen3_url' in data) { jQuery('#importscreen3').val( data.screen3_url ); }
      if ( 'screen4_url' in data) { jQuery('#importscreen4').val( data.screen4_url ); }
    }
  }
  else {
    if ( statusText !== 'success' ) {
...
function showResponseUnity(data, statusText, xhr, $form)  {
  ...
  // Check the status
  if (statusText == 'success' && data.error == '') {
    jQuery('#filenameunity').html('<strong>' + data.name + '</strong>');
    jQuery('#gamename').val(data.realname);
    jQuery('#importgame').val(data.location_url);
    jQuery('#importtype').val(data.type);
  }
...
function myarcade_check_file_size( fileID, targetID ) {
  ...
  var allowedSize = <?php echo F4ED61B12378CBA912A59EE6E1BC7D883(); ?>;
  var unit      = "kB";
